...
try {
    ...
    if (isset($data['email'], $data['userId'], $data['localisation'],$data['cin'], $data['adresse'], $data['nom'], $data['prenom'], $data['date'])) {
        ...
        $sql = $pdo->prepare('INSERT INTO client(cin, nom, prenom, addresse, localisation, id_vendeur, id_commerciale, date_naissance,telephone_client) VALUES(?,?,?,?,?,?,?,?,?)');
        $sql->execute([$cin, $nom, $prenom, $adresse, $localisation, $id_vend, $id_com, $date,$tel]);
