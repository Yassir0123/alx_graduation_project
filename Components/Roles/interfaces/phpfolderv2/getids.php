...
try {
    ...
    if ($domain == "vendeur.com") {
        $sql = $pdo->prepare('SELECT id_client,nom,prenom,localisation FROM client WHERE id_vendeur=?');
    } else {
        $sql = $pdo->prepare('SELECT id_client,nom,prenom,localisation FROM client WHERE id_commerciale=?');
    }
    $sql->execute([$id]);
