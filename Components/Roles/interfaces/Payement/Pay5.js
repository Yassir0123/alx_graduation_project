...
const DeliveryOptions = () => {
  ...
  useEffect(() => {
    if (Bonlivraison) {
      axios.post(
        'http://192.168.0.48/logo/Components/Roles/interfaces/phpfolderv2/getprodbybonimg.php',
  ...
  const deliveryOptions = [
    ...
    { id: 'subway', icon: 'subway-variant', label: 'Subway Pickup', detail: 'Pick up your order at the nearest subway station.' },
    { id: 'store', icon: 'store', label: 'Store Pickup', detail: 'Collect your order from the nearest store.' },
  ];
        ...
        {products.map((product) => (
          <TouchableOpacity key={product.id} style={styles.productContainer} onPress={() => navigation.navigate('OrderItemsScreen', { products: [product], Bonlivraison })}>
            <View style={styles.productCard}>
              <Image source={{ uri: `http://192.168.0.48/logo/Components/Roles/interfaces/Products/${product.image}` }} style={styles.productImage} />
