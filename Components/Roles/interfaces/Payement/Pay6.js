  ...
  const handlepress = async () => {
    try {
      ...
      const bonvaliderData = {
        ...
        id_vendeur: Bonlivraison.id_vendeur,
        id_client: Bonlivraison.id_client,
        nom_client: clientDetails.nom,
        prenom_client: clientDetails.prenom,
      ...
      const response = await axios.post(
        'http://192.168.0.48/logo/Components/Roles/interfaces/phpfolderv2/addpays.php',
      ...
      if (response.data.message === 'gotdata') {
        navigation.navigate('Pay7', {
          id_bonlivraison: Bonlivraison.id_bonlivraison,
          totalTVA: totalTVA,
          totalTax: tax,  // Pass total tax if needed
        });
      } else {
          ...
          <Card style={styles.card}>
            <Card.Content>
              ...
              <List.Item
                ...
                left={props => <MaterialCommunityIcons name="receipt" size={24} color={theme.colors.primary} />}
              />
              <List.Item
                title="Tax"
                description={`$${formatAmount(tax)}`}
                left={props => <MaterialCommunityIcons name="percent" size={24} color={theme.colors.primary} />}
              />
            </Card.Content>
          </Card>
