  ...
  const handleAdd = async () => {
    try {
      ...
      const fetchedProductData = response.data;
       ...
       //       console.log('FournisseurData.fournisseur_id:', FournisseurData.id_fournisseur);
const filteredProducts = fetchedProductData.filter(product => {
  return product.id_fournisseur === FournisseurData.id_fournisseur;
});

      // console.log(filteredProducts.fournisseur_id);
  
      // Now you can set the filtered products in your state
 ...
 //    console.log(productData);
  ...
  setids(product.id_produit);
   // setEditingProduct(product);
    setEditedData({
      id:product.id_lignecommandeachat,
...
//    console.log(addedProduct);
  ...
  const handlevalidate=async()=>{
    ...
    const response = await axios.post(
      'http://192.168.11.105/logo/Components/Roles/interfaces/phpfolderv2/validatecmdachat.php',
      {
     ...
     if(response.data.message==='Data inserted successfully'){
      navigation.navigate('Message',{emails:FournisseurData.email_contact,idcmd:FournisseurData.idcmdcount});
     }else{
...
const formattedData = productData.map((item, index) => ({
  ...
  // console.log(FournisseurData);
         if(FournisseurData.action===0){
          const response = await axios.post(
            'http://192.168.11.105/logo/Components/Roles/interfaces/phpfolderv2/deletecmdachat.php',
            {
  ...
  if(FournisseurData.Data!=null){
 setDateText(FournisseurData.date_livraison);
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.user}>
        <Text>Commande {FournisseurData.idcmdcount}</Text>
        <Text>{FournisseurData.nom_entreprise}</Text>
      </View>
...
{productDatas && productDatas.length > 0 ? (
  productDatas.map((item, index) => (
    <View style={styles.tableRow} key={index}>
      ...
      <TouchableOpacity
        style={styles.tableCell}
        onPress={() => Delete(item.id_lignecommandeachat)}
      >
...
export default FournisseursInfos;
