...
const CommandScreen = ({ route }) => {
  ...
  const handlepress = async () => {
    ...
    console.log(email); 
    const response = await axios.post(
    'http://192.168.11.106/alx/alx/Components/Roles/interfaces/phpfolderv2/validatecmd.php',
      {
  ...
  const saveEditedProduct = (editedProduct) => {
    ...
    if (editedProduct.quantiter > matchedProduct.quantiter_stock+oldqt) {
      ...
      const editedProd = [
        editedProduct.id_lignecommande,
        editedProduct.quantiter,
        editedProduct.id_produit,
        matchedProduct.quantiter_stock,
      ];
