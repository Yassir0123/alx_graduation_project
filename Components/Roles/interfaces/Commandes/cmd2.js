...
const Item = ({ id, name, price, quantity, img, tva, onAddButtonPress }) => (
  <View style={styles.productCard}>
    <View style={styles.imageContainer}>
      <Image source={{ uri: `http://192.168.0.48/logo/Components/Roles/interfaces/Products/${img}` }} style={styles.productImage} />
...
export default function App() {
    ...
    const fetchClientData = async () => {
      try {
        const produitsResponse = await axios.post(
          'http://10.20.69.188/logo/Components/Roles/interfaces/phpfolderv2/getallproducts.php',
          { responseType: 'json' }
        );
        const subcategoriesResponse = await axios.post(
          'http://10.20.69.188/logo/Components/Roles/interfaces/phpfolderv2/getcategories.php',
          { responseType: 'json' }
        );
        const maincategoriesResponse = await axios.post(
          'http://10.20.69.188/logo/Components/Roles/interfaces/phpfolderv2/getcategoriesgrand.php',
          { responseType: 'json' }
        );
  ...
  const handleAddButtonPress = (product) => {
    ...
    navigation.navigate('PlaceCommand',{clientData,product,selectedMainCategory,selectedSubCategory});
  };
