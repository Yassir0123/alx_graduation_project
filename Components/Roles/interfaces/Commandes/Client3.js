      ...
      try {
        ...
        if (userId !== null && email !== null) {
          ...
          const response = await axios.post(
            'http://10.20.69.188/logo/Components/Roles/interfaces/phpfolderv2/getids.php',
            {
              userId: userId,
              email: email,
            },
  ...
  const handleUser = async () => {
    if (selectedOption) {
      ...
      if (selectedClient) {
        ...
        navigation.navigate('Commande',{clientData : selectedClient});
     //    navigation.navigate('Ajouter_un_Produits', { clientData: selectedClient });
      }
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.form}>
        <Text style={styles.text}>N°Client</Text>
        <Picker
          style={styles.picker}
          ...
          {data.map((option) => (
            <Picker.Item
              ...
              label={option.id_client.toString()}
